...
Rcpp::List sampleMVN (
  ...
  arma::cube mean_sum_saved(P, K * B, n_saved),
    ...
    // Interaction term (flattened P x (K*B) per iteration, same convention
    // as mean_sum_saved) and GP-correlated batch weights; entirely zero /
    // uniform if include_interaction / weight_prior_type are off.
  ...
  // Snapshots of the acceptance counters, used to compute each sweep's
  // realised acceptance rate for Robbins-Monro proposal-window adaptation
  // (robbinsMonroUpdate(), genericFunctions.cpp/.h); frozen once r >= n_burn.
