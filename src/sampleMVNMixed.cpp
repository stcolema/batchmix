...
// =============================================================================
// sampleMVNMixed function implementation
...
Rcpp::List sampleMVNMixed(
  ...
  // The completed (observed + augmented latent) data at each saved
  // iteration, on the shared latent-Gaussian scale used internally; for
  // binary columns this is a continuous liability score, not {0, 1}.
  latent_data(my_sampler.N, P, n_saved),
  ...
  arma::uvec prev_r_count = my_sampler.r_count;
  arma::uvec prev_sigma_count = my_sampler.sigma_count;
  ...
  for(arma::uword r = 0; r < R; r++){
    ...
    // Complete the data given the current parameters (Albert & Chib, 1993;
    // Chib & Greenberg, 1998; Dunson, 2000) before anything that treats
    // X/X_t as fully-observed continuous data runs.
    ...
    prev_sigma_count = my_sampler.sigma_count;
