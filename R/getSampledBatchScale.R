...
#'
#' batch_scale_df <- getSampledBatchShift(samples$batch_scale, n_iter = n_iter, thin = thin)
#'
...
getSampledBatchScale <- function(sampled_batch_scale,
                                 ...
                                 n_iter = dim(sampled_batch_scale)[3],
                                 thin = 1) {
  # Check that the values of n_iter and thin make sense
  if (floor(n_iter / thin) != dim(sampled_batch_scale)[3]) {
