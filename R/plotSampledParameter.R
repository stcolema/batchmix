...
#' @param n_iter The number of iterations run. Defaults to the number of samples for
...
#' samples <- runBatchMix(X, n_iter, thin, batch_vec, "MVN", K_max = 8)
#'
#' # Plot the sampled value of the cluster means against MCMC iteration
#' parameter <- "means"
#' plotSampledParameter(samples, parameter, n_iter, thin)
#' @importFrom ggplot2 ggplot aes geom_point facet_wrap labs
plotSampledParameter <- function(samples, parameter, n_iter = NULL, thin = 1, burn_in = 0) {
  n_param <- dim(samples[[parameter]])[2]
  P <- dim(samples[[parameter]])[1]

  if (is.null(n_iter)) {
    n_iter <- nrow(samples$samples)
  }
  ...
  sampled_parameter <- getSampledBatchShift(samples[[parameter]], n_param, P, n_iter = n_iter, thin = thin)
