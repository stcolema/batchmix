...
#' \code{X} entry).
#' @param style One of \code{"density"} or \code{"statistic"}.
...
plotPredictiveCheck <- function(X,
  ...
  if (style == "density") {
    ...
    obs_df <- data.frame(value = X[, column])
  ...
  obs_stat <- statistic(X[, column])
  rep_stats <- vapply(replicates, function(r) statistic(r$X[, column]), numeric(1))
